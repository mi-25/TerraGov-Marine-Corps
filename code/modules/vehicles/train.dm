...
/obj/vehicle/train/proc/attach_to(obj/vehicle/train/T, mob/user, silent=FALSE)
	...
	if (get_dist(src, T) > 1)
		if(!silent)
			to_chat(user, span_warning("[src]距离[T]太远，无法将它们连接在一起。"))
		return

	if (lead)
		if(!silent)
			to_chat(user, span_warning("[src] 已经连接到某个物体上了。"))
		return
	...
	while (next_car)
		if (next_car == src)
			if(!silent)
				to_chat(user, span_warning("这听起来很蠢。"))
			return
	...
	if(user && !silent)
		to_chat(user, span_notice("你将[src]挂载到[T]上。"))
...
/obj/vehicle/train/proc/unattach(mob/user, silent=FALSE)
	...
	if (!lead)
		if(!silent)
			to_chat(user, span_warning("[src] 没有拴在任何东西上。"))
		return

	lead.tow = null

	if(!silent)
		to_chat(user, span_notice("你将[src]从[lead]身上解下。"))
	lead = null
