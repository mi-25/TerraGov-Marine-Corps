...
/obj/structure/mecha_wreckage
	name = "外骨骼残骸"
	desc = "某台不幸机甲的残骸。已完全无法修复，但或许还能回收些零件。"
...
/obj/structure/mecha_wreckage/welder_act(mob/living/user, obj/item/I)
	...
	if(salvage_num <= 0 || !length(welder_salvage))
		to_chat(user, span_notice("你看不到任何能用[I]切割的东西！"))
		return
	...
	user.visible_message(span_notice("[user]从[src]上切下[N]。"), span_notice("You cut [N] from [src]."))
...
/obj/structure/mecha_wreckage/crowbar_act(mob/living/user, obj/item/I)
	...
	if(length(crowbar_salvage))
		...
		user.visible_message(span_notice("[user]从[src]撬下了[S]。"), span_notice("You pry [S] from [src]."))
		crowbar_salvage -= S
		return
	to_chat(user, span_notice("你看不到任何能用[I]切割的东西！"))
