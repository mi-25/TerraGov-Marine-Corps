	...
	switch(mode)
		if(SYRINGE_DRAW)
			...
			if(reagents.holder_full())
				to_chat(user, span_warning("注射器已满。"))
				return
			...
			if(ismob(target))//Blood!
				if(iscarbon(target))//maybe just add a blood reagent to all mobs. Then you can suck them dry...With hundreds of syringes. Jolly good idea.
					...
					if(T.get_blood_id() && reagents.has_reagent(T.get_blood_id()))
						...
						return
					if(!T.blood_type)
						to_chat(user, span_warning("你无法找到任何血迹。"))
						return
				...
				if(!target.reagents.total_volume)
					to_chat(user, "<span class='warning'>[target]是空的。</span>")
					return

				if(!target.is_drawable())
					to_chat(user, span_warning("你无法直接从此物体中移除试剂。"))
					return
		...
		if(SYRINGE_INJECT)
			...
			if(!target.is_injectable() && !ismob(target))
				to_chat(user, span_warning("你无法直接填充此物体。"))
				return
			...
			if(ismob(target))
				if(target != user)
					...
					if(injection_time != 6 SECONDS)
						user.visible_message(span_danger("[user] 正试图给 [target] 注射！"))
					else
					...
					user.visible_message(span_warning("[user] 给 [target] 注射了注射器里的药剂！"))
			...
			to_chat(user, span_notice("你注射了[trans]单位溶液。注射器现在含有[src.reagents.total_volume]单位。"))
...
/obj/item/reagent_containers/syringe/proc/syringestab(mob/living/carbon/target as mob, mob/living/carbon/user as mob)
	...
	if(ishuman(target))
		...
		if (target != user && !prob(target.modify_by_armor(100, MELEE, penetration, target_zone)))
			visible_message(span_danger("[user]试图用[src]刺向[target]的\the [hit_area]，但攻击被装甲弹开了！"))
	...
	switch(mode)
		if(SYRINGE_DRAW)
			...
			if(ismob(target))
				if(iscarbon(target))//I Do not want it to suck 50 units out of people
					to_chat(usr, span_warning("这根针头不是用来抽血的。"))
					return
			else //if not mob
				if(!target.reagents.total_volume)
		...
		if(SYRINGE_INJECT)
			...
			if(ismob(target) && target != user)
				user.visible_message(span_danger("[user]正试图用巨型注射器给[target]注射！"))
				if(!do_after(user, 30 SECONDS, NONE, target, BUSY_ICON_DANGER, BUSY_ICON_DANGER))
					return
				user.visible_message(span_warning("[user] 用一支巨大的注射器给 [target] 注射！"))
			spawn(5)
