...
/obj/item/reagent_containers/borghypo/attack(mob/living/M as mob, mob/user as mob)
	...
	to_chat(user, span_notice("你给[M]注射了注射器。"))
	to_chat(M, span_notice("[user] 给你注射了注射器。"))
	...
	if(M.reagents)
		...
		// to_chat(user, span_notice("[t]单位已注射。[reagent_volumes[reagent_ids[mode]]]单位剩余。"))
		to_chat(user, span_notice("[t] 单位的 <span class='warning'> [reagent_ids[mode]] <span class='notice'> 已注入，总计 <span class='warning'> [round(M.reagents.get_reagent_amount(reagent_ids[mode]))]<span class='notice'>。剩余 [reagent_volumes[reagent_ids[mode]]] 单位。"))
...
/obj/item/reagent_containers/borghypo/attack_self(mob/user)
	...
	to_chat(user, span_notice("合成器正在生产'[R.name]'。"))
