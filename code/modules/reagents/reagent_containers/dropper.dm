...
/obj/item/reagent_containers/dropper/afterattack(obj/target, mob/user , flag)
	...
	if(filled)
		...
		if(ismob(target))
			...
			visible_message(span_danger("[user]正试图往[target]的眼睛里喷射什么东西！"))
			...
			if(ishuman(target))
				...
				if(safe_thing)
					...
					visible_message(span_danger("[user]试图朝[target]的眼睛喷射什么东西，但失败了！"))
			...
			visible_message(span_danger("[user] 朝 [target] 的眼睛里喷了什么东西！"))
		...
		balloon_alert(user, "吸取 [trans] 单位的液体")
