...
/obj/machinery/power/smes/batteryrack/substation
	name = "变电站电源"
	desc = "一组作为电源单元工作的电池组。这个似乎配备了更高的功率负载。"
	output_level = 150000
	input_level = 150000
...
/obj/machinery/power/smes/batteryrack/attackby(obj/item/I, mob/user, params) //these can only be moved by being reconstructed, solves having to remake the powernet.
	...
	if(iscrowbar(I))
		if(charge >= (capacity / 100))
			to_chat(user, span_warning("最好先让[src]放电再拆卸。"))
			return

		if(outputting || input_attempt)
			to_chat(user, span_warning("在拆卸前关闭[src]。"))
			return
		...
		if(charge >= (capacity / 100))
			to_chat(user, span_warning("最好让[src]先放电，然后再把手伸进去。"))
			return
