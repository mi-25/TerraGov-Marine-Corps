...
/obj/item/clothing/under/proc/set_sensors(mob/living/user)
	...
	if (user.incapacitated(TRUE))
		return
	if(has_sensor >= 2)
		to_chat(user, "[src]的传感器无法修改。")
		return FALSE
	if(has_sensor <= 0)
		to_chat(user, "[src] 没有安装任何传感器。")
		return FALSE
	...
	if (loc == user)
		switch(sensor_mode)
			if(0)
				to_chat(user, "你关闭了你的远程传感设备。")
			if(1)
				to_chat(user, "你的防护服现在会报告你是存活还是死亡。")
			if(2)
				to_chat(user, "你的作战服现在将报告你的生命体征。")
			if(3)
				to_chat(user, "你的作战服现在将报告你的生命体征以及坐标位置。")
	else if (ismob(loc))
		switch(sensor_mode)
			if(0)
				visible_message(span_warning("[user] 禁用了 [loc] 的远程传感设备。"), null, null, 1)
			if(1)
				visible_message("[user] 将 [loc] 的远程传感器切换至二进制模式。", null, null, 1)
			if(2)
				visible_message("[user] 将 [loc] 的传感器设置为追踪生命体征。", null, null, 1)
			if(3)
				visible_message("[user] 将 [loc] 的传感器设置为最大值。", null, null, 1)
...
/obj/item/clothing/under/verb/rollsuit()
	...
	if(!length(adjustment_variants))
		to_chat(usr, span_warning("你无法卷起制服！"))
		return
	var/variant = null
