...
/obj/machinery/computer/camera_advanced/shuttle_docker/minidropship/proc/take_off()
	...
	if(TIMER_COOLDOWN_RUNNING(src, COOLDOWN_TADPOLE_LAUNCHING))
		to_chat(ui_user, span_warning("运输机的引擎尚未就绪"))
		return
...
/obj/machinery/computer/camera_advanced/shuttle_docker/minidropship/attack_alien(mob/living/carbon/xenomorph/xeno_attacker, damage_amount = xeno_attacker.xeno_caste.melee_damage, damage_type = BRUTE, armor_type = MELEE, effects = TRUE, armor_penetration = xeno_attacker.xeno_caste.melee_ap, isrightclick = FALSE)
	...
	if(HAS_TRAIT_FROM(xeno_attacker, TRAIT_TURRET_HIDDEN, STEALTH_TRAIT))
		return
	xeno_attacker.visible_message("[xeno_attacker] 开始小心翼翼地斩击电脑",
	...
	if(!do_after(xeno_attacker, 10 SECONDS, NONE, src, BUSY_ICON_DANGER, BUSY_ICON_HOSTILE))
		return
	visible_message("内部线路暴露在外，可以斩击！")
	xeno_attacker.visible_message("[xeno_attacker] 继续斩击着电脑",
	...
	if(!do_after(xeno_attacker, 10 SECONDS, NONE, src, BUSY_ICON_DANGER, BUSY_ICON_HOSTILE))
		return
	visible_message("线路被摧毁了，没人能修好这台电脑！")
	...
	if(fly_state == SHUTTLE_IN_ATMOSPHERE && last_valid_ground_port)
		visible_message("自动驾驶检测到舵机控制失效。正在启动紧急着陆！")
	...
	if(next_fly_state == SHUTTLE_IN_ATMOSPHERE)
		...
		visible_message("自动驾驶检测到舵机控制丢失。中止起飞！")
...
/obj/machinery/computer/attackby(obj/item/I, mob/user, params)
	...
	if(!(machine_stat & BROKEN))
		to_chat(user,span_notice("电路不需要更换"))
		return
	playsound(loc, 'sound/items/ratchet.ogg', 25, 1)
	if(user.skills.getRating(SKILL_ENGINEER) < SKILL_ENGINEER_EXPERT)
		user.visible_message(span_notice("[user] 笨手笨脚地摸索着如何更换电子元件。"),
...
/datum/action/innate/shuttledocker_land/Activate()
	...
	if(!origin.placeLandingSpot(target))
		to_chat(owner, span_warning("你无法在此处降落。"))
		return
	if(is_ground_level(origin.z)) //Safety check to prevent instant transmission
		to_chat(owner, span_warning("穿梭机在行星上停靠时无法移动"))
		return
