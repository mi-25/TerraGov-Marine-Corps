...
/mob/living/carbon/proc/do_give()
	if(stat != CONSCIOUS)
		return
	if(!hand && r_hand == null)
		to_chat(usr, span_warning("你右手没有东西可以给。"))
		return
	if(hand && l_hand == null)
		to_chat(usr, span_warning("你左手里没有东西可以给。"))
		return
	...
	if(to_give_to.r_hand && to_give_to.l_hand)
		to_chat(src, span_warning("[to_give_to]的手已经满了。"))
		return
	if(tgui_alert(to_give_to, "[src] wants to give you \a [item]?", null, list("Yes","No")) != "Yes")
		return
	if(!Adjacent(to_give_to))
		to_chat(src, span_warning("给予物品时，你需要保持在可触及的距离内。"))
		to_chat(to_give_to, span_warning("[src]移动距离过远。"))
		return
	if((hand && l_hand != item) || (!hand && r_hand != item))
		to_chat(src, span_warning("你需要将物品保持在主手中。"))
		to_chat(to_give_to, span_warning("[src]似乎已经放弃把[item]给你了。"))
		return
	if(to_give_to.r_hand != null && to_give_to.l_hand != null)
		to_chat(src, span_warning("[to_give_to]的手已经满了。"))
		to_chat(to_give_to, span_warning("你的双手已满。"))
		return
	if(!drop_held_item())
		return
