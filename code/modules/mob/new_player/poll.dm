...
/mob/new_player/proc/poll_player(datum/poll_question/poll)
	if(!poll)
		return
	if(!SSdbcore.Connect())
		to_chat(src, span_danger("数据库连接失败。"))
		return
...
/mob/new_player/proc/vote_on_poll_handler(datum/poll_question/poll, href_list)
	...
	if(IsAdminAdvancedProcCall())
		...
		to_chat(src, span_danger("处理您的投票时出现严重错误。请联络管理员，他们应该已收到相关通知。"))
		return
	var/admin_rank
	if(client.holder)
	...
	if(query_validate_poll_vote.NextRow())
		...
		if(vote_id && !poll.allow_revoting)
			to_chat(usr, span_danger("投票重新投票已禁用，您已回复此投票。"))
			qdel(query_validate_poll_vote)
			return
	else
...
/mob/new_player/proc/vote_on_poll_rating(datum/poll_question/poll, list/href_list, admin_rank, sql_poll_id)
	...
	for(var/h in href_list)
		...
		if (!option)
			to_chat(src, span_warning("无效票已从您的选票中剔除，请重新投票。"))
		sql_votes += list(list(
...
/mob/new_player/proc/vote_on_poll_multi(datum/poll_question/poll, list/href_list, admin_rank, sql_poll_id)
	...
	for(var/h in href_list)
		if(vote_count == poll.options_allowed)
			to_chat(src, span_danger("允许的选项数量超出限制，仅保存了前 [poll.options_allowed] 个选中的选项。"))
			break
		vote_count++
...
/mob/new_player/proc/vote_on_poll_irv(datum/poll_question/poll, list/href_list, admin_rank, sql_poll_id)
	...
	if(!length(votelist))
		to_chat(src, span_danger("未找到订购数据。请重试或联系管理员。"))
