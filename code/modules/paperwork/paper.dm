...
/obj/item/paper/attack(mob/living/carbon/M, mob/living/carbon/user)
	if(user.zone_selected == "eyes")
		user.visible_message(span_notice("你向[M]展示了这张纸。"), \
	...
	if(user.zone_selected == "head") // face paint wiping
		...
		user.visible_message(span_warning("[user]开始用\the [src]擦掉[H]的脸部涂装。"), \
							span_notice("You begin to wipe off [H]'s face paint."))
		if(!do_after(user, 10, NONE, H, BUSY_ICON_FRIENDLY))
			return
		user.visible_message(span_notice("[user]用\the [src]擦掉了[H]脸上的迷彩。"), \
...
/obj/item/paper/proc/burnpaper(obj/item/P, mob/user)
	...
	if(P.heat >= 400 && !user.restrained())
		...
		user.visible_message("[class][user] 将 \the [P] 举到 \the [src] 前，看起来 [user.p_theyre()] 想烧掉它！</span>", \
		"[class]You hold \the [P] up to \the [src], burning it slowly.</span>")

		spawn(20)
			if(get_dist(src, user) < 2 && user.get_active_held_item() == P && P.heat)
				user.visible_message("[class][user] 将 \the [src] 彻底烧穿，将其化为灰烬。它在空中飘散，最终落在地面堆积成一小堆。", \
				...
				to_chat(user, span_warning("你必须稳住 \the [P] 才能烧灼 \the [src]。"))
	...
	if(istype(I, /obj/item/paper) || istype(I, /obj/item/photo))
		if(istype(I, /obj/item/paper/carbon))
			...
			if(!C.iscopy && !C.copied)
				to_chat(user, span_notice("先把复写纸拿掉。"))
				return
		if(loc != user)
			return
		...
		user.dropItemToGround(src)
		to_chat(user, span_notice("你将\the [I]夹在[src]上。"))
		...
		to_chat(user, span_notice("你给文件盖上了你的橡皮图章。"))
