...
/datum/admins/proc/edit_rights_topic(list/href_list)
	...
	if(IsAdminAdvancedProcCall())
		to_chat(usr, "<span class='admin prefix'>管理员编辑被阻止：检测到高级过程调用。</span>")
		return
	...
	if(!CONFIG_GET(flag/admin_legacy_system) && CONFIG_GET(flag/protect_legacy_admins) && task == "rank")
		if(new_admin_ckey in GLOB.protected_admins)
			to_chat(usr, "<span class='admin prefix'>编辑此管理员的权限等级被服务器配置阻止。</span>")
			return
	if(!CONFIG_GET(flag/admin_legacy_system) && CONFIG_GET(flag/protect_legacy_ranks) && task == "permissions")
		if(D.rank in GLOB.protected_ranks)
			to_chat(usr, "<span class='admin prefix'>编辑此军衔的权限已被服务器配置阻止。</span>")
			return
	if(CONFIG_GET(flag/load_legacy_ranks_only) && (task == "add" || task == "rank" || task == "permissions"))
		to_chat(usr, "<span class='admin prefix'>数据库军衔加载已禁用，仅可对军衔权限进行临时更改，永久创建新军衔功能已被阻止。</span>")
		legacy_only = TRUE
	if(check_rights(R_DBRANKS, FALSE))
		if(!skip)
			if(!SSdbcore.Connect())
				to_chat(usr, span_danger("无法连接到数据库，更改仅为临时保存。"))
				use_db = FALSE
			else
...
/datum/admins/proc/add_admin(admin_ckey, admin_key, use_db)
	...
	if(!admin_ckey && (. in (GLOB.admin_datums+GLOB.deadmins)))
		to_chat(usr, span_danger("[admin_key] 已经是管理员了。"))
		return FALSE
	if(use_db)
		...
		if(query_admin_in_db.NextRow())
			qdel(query_admin_in_db)
			to_chat(usr, span_danger("[admin_key] 已存在于管理员数据库中。如果未出现在管理员列表中，请检查管理选项卡。"))
			return FALSE
		qdel(query_admin_in_db)
...
/datum/admins/proc/remove_rank(admin_rank)
	...
	for(var/datum/admin_rank/R in GLOB.admin_ranks)
		if(R.name == admin_rank && (!(R.rights & usr.client.holder.rank.can_edit_rights) == R.rights))
			to_chat(usr, "<span class='admin prefix'>您没有编辑此军衔所有权限的权限，不允许删除军衔。</span>")
			return
	if(!CONFIG_GET(flag/admin_legacy_system) && CONFIG_GET(flag/protect_legacy_ranks) && (admin_rank in GLOB.protected_ranks))
		to_chat(usr, "<span class='admin prefix'>禁止删除受保护的管理员等级，必须从 admin_ranks.txt 文件中移除。</span>")
		return
	if(CONFIG_GET(flag/load_legacy_ranks_only))
		to_chat(usr, "<span class='admin prefix'>数据库军衔加载已禁用时，不允许删除军衔。</span>")
		return
	...
	if(query_admins_with_rank.NextRow())
		qdel(query_admins_with_rank)
		to_chat(usr, span_danger("错误：尝试删除仍在使用的军衔；请告知程序员，这不应该发生。"))
		return
	qdel(query_admins_with_rank)
