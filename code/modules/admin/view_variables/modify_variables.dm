...
/client/proc/mod_list_add(list/L, atom/O, original_name, objectvar)
	...
	if (O)
		if (O.vv_edit_var(objectvar, L) == FALSE)
			to_chat(src, "你的编辑被目标拒绝了。", confidential = TRUE)
			return
...
/client/proc/mod_list(list/L, atom/O, original_name, objectvar, index, autodetect_class = FALSE)
	...
	if(default == VV_NUM)
		...
		if(dir_text)
			to_chat(usr, "如果是一个方向，方向为：[dir_text]", confidential = TRUE)
...
/client/proc/modify_variables(atom/O, param_var_name = null, autodetect_class = 0)
	...
	if(param_var_name)
		if(!(param_var_name in O.vars))
			to_chat(src, "此数据对象 ([O]) 中不存在名为 ([param_var_name]) 的变量", confidential = TRUE)
			return
