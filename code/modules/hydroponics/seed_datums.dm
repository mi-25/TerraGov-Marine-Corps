...
/datum/seed/proc/mutate(degree,turf/source_turf)
	...
	source_turf.visible_message(span_notice("\The [display_name] 在颤抖！"))
	...
	for(var/i = 0;i<total_mutations;i++)
		switch(rand(0,11))
			if(0) //Plant cancer!
				...
				source_turf.visible_message(span_warning("\The [display_name] 迅速枯萎了！"))
			if(1)
			...
			if(5)
				...
				if(prob(degree*5))
					...
					if(carnivorous)
						source_turf.visible_message(span_notice("\The [display_name] 饥渴地颤抖着。"))
			if(6)
				weed_tolerance = max(0,  min(10,  weed_tolerance       + (rand(-2,2)   * degree)))
			...
			if(8)
				...
				if(prob(degree*5))
					...
					source_turf.visible_message(span_notice("\The [display_name] 明显痉挛起来，在托盘里扭动。"))
			if(9)
			...
			if(10)
				if(prob(degree*2))
					biolum = !biolum
					if(biolum)
						source_turf.visible_message(span_notice("\The [display_name]开始发光！"))
						if(prob(degree*2))
							biolum_colour = "#[pick(list("FF0000","FF7F00","FFFF00","00FF00","0000FF","4B0082","8F00FF"))]"
							source_turf.visible_message(span_notice("\The [display_name]的发光<font color='[biolum_colour]'>改变了颜色</font>！"))
					else
						source_turf.visible_message(span_notice("\The [display_name]的光芒暗淡了..."))
			if(11)
				if(prob(degree*2))
					flowers = !flowers
					if(flowers)
						source_turf.visible_message(span_notice("\The [display_name] 绽放出一簇鲜花！"))
						if(prob(degree*2))
							flower_colour = "#[pick(list("FF0000","FF7F00","FFFF00","00FF00","0000FF","4B0082","8F00FF"))]"
						source_turf.visible_message(span_notice("\The [display_name]的花朵<font=[flower_colour]>改变了颜色</font>！"))
					else
						source_turf.visible_message(span_notice("\The [display_name]的花朵枯萎凋零了。"))
	...
	if(!got_product && !harvest_sample)
		...
		for(var/i = 0;i<total_yield;i++)
			...
			if(istype(product,/mob/living))
				product.visible_message(span_notice("舱体弹出了[product]！"))
