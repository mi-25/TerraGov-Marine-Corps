...
/obj/machinery/door/airlock/AICtrlClick(mob/living/silicon/ai/user) // Bolts doors
	if(aiControlDisabled)
		to_chat(user, span_notice("[src] 的AI远程控制已禁用。"))
		return
	if(emergency)
		to_chat(user, span_notice("紧急访问状态下的门无法上锁。"))
		return
	if(locked)
		bolt_raise(user)
	else if(hasPower())
...
/obj/machinery/door/airlock/AIShiftClick(mob/living/silicon/ai/user)  // Opens and closes doors!
	if(aiControlDisabled)
		to_chat(user, span_notice("[src] 的AI远程控制已被禁用。"))
		return
...
/obj/machinery/door/airlock/AICtrlShiftClick(mob/living/silicon/ai/user)
	if(aiControlDisabled)
		...
		return
	if(locked || !hasPower())
		to_chat(user, span_notice("紧急访问机制无法访问。"))
		return
	if(emergency)
		to_chat(user, span_notice("[src] 紧急访问权限已被禁用。"))
		emergency_off(user)
	else
		to_chat(user, span_notice("[src] 紧急访问权限已启用。"))
		emergency_on(user)
...
/turf/AIShiftClick(mob/living/silicon/ai/user)
	...
	if(istype(A) && A.ceiling >= CEILING_UNDERGROUND)
		to_chat(user, span_warning("你无法命中目标。目标可能在地下。"))
		return
	to_chat(user, span_notice("正在向[user.linked_artillery]发送目标信息。坐标：X:[x] Y:[y]"))
...
/turf/AICtrlShiftClick(mob/living/silicon/ai/user)
	...
	if(HAS_TRAIT(user, TRAIT_IS_FIRING_RAILGUN))
		to_chat(user, span_warning("导轨炮已在瞄准一个位置，等待它们完成。"))
		return
	...
	if((GLOB.rail_gun?.last_firing_ai + COOLDOWN_RAILGUN_FIRE) > world.time)
		...
		return
	to_chat(user, span_notice("正在向[src]发射轨道导轨炮，坐标：X:[x] Y:[y]"))
