...
/datum/controller/subsystem/vote/proc/result(default_result)
	...
	if(restart)
		...
		if(!active_admins)
			...
			to_chat(world, "<span style='boltnotice'>注意：重启投票不会自动重启服务器，因为有管理员在线。</span>")
...
/datum/controller/subsystem/vote/proc/initiate_vote(vote_type, initiator_key, ignore_delay = FALSE, popup_override = FALSE)
	...
	if(!mode)
		if(started_time && !ignore_delay)
			...
			if(mode)
				to_chat(usr, span_warning("已有投票正在进行中！请等待其结束。"))
				return FALSE
			if(next_allowed_time > world.time && !lower_admin)
				to_chat(usr, span_warning("最近已发起过投票，必须等待 [DisplayTimeText(next_allowed_time-world.time)] 才能发起新的投票！"))
				return FALSE
		...
		switch(vote_type)
			...
			if("groundmap")
				...
				if(!lower_admin && SSmapping.groundmap_voted)
					to_chat(usr, span_warning("下一张地面地图已选定。"))
					return FALSE
			...
			if("shipmap")
				...
				if(!lower_admin && SSmapping.shipmap_voted)
					to_chat(usr, span_warning("下一艘舰船地图已选定。"))
					return FALSE
				var/datum/game_mode/next_gamemode = config.pick_mode(trim(file2text("data/mode.txt")))
				var/list/maps = list()
				if(!config.maplist)
			...
			if("custom")
				...
				return FALSE
		if(!length(choices))
			to_chat(usr, span_warning("该投票没有可选项"))
			reset()
			return FALSE
		mode = vote_type
		if(length(choices) == 1)
