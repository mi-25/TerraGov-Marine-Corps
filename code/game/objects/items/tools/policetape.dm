//Define all tape types in policetape.dm
/obj/item/tool/taperoll
	name = "胶带卷"
	icon = 'icons/obj/policetape.dmi'
	icon_state = "rollstart"
	item_flags = NOBLUDGEON
	w_class = WEIGHT_CLASS_SMALL
	var/turf/start
	var/turf/end
	var/tape_type = /obj/item/tape
	var/icon_base

/obj/item/tape
	name = "胶带"
	icon = 'icons/obj/policetape.dmi'
	anchored = TRUE
	var/lifted = 0
	var/crumpled = 0
	var/icon_base

/obj/item/tape/Initialize(mapload)
	. = ..()
	var/static/list/connections = list(
		COMSIG_ATOM_ENTERED = PROC_REF(on_cross),
	)
	AddElement(/datum/element/connect_loc, connections)

/obj/item/tool/taperoll/police
	name = "警戒带"
	desc = "一卷用于封锁犯罪现场、禁止公众进入的警戒带。"
	icon_state = "police_start"
	tape_type = /obj/item/tape/police
	icon_base = "police"

/obj/item/tape/police
	name = "警戒带"
	desc = "一段警戒带。请勿跨越。"
	req_access = list(ACCESS_MARINE_BRIG)
	icon_base = "police"

/obj/item/tool/taperoll/engineering
	name = "工程胶带"
	desc = "一卷工程胶带，用于将工作区域与公众隔离开来。"
	icon_state = "engineering_start"
	tape_type = /obj/item/tape/engineering
	icon_base = "engineering"

/obj/item/tape/engineering
	name = "工程胶带"
	desc = "一段工程胶带。最好不要跨越它。"
	req_one_access = list(ACCESS_MARINE_ENGINEERING, ACCESS_MARINE_ENGPREP)
	icon_base = "engineering"

/obj/item/tool/taperoll/attack_self(mob/user as mob)
	if(icon_state == "[icon_base]_start")
		start = get_turf(src)
		to_chat(usr, span_notice("你将[src]的第一端放置好。"))
		icon_state = "[icon_base]_stop"
	else
		icon_state = "[icon_base]_start"
		end = get_turf(src)
		if(start.y != end.y && start.x != end.x || start.z != end.z)
			to_chat(usr, span_notice("[src]只能水平或垂直放置。"))
			return

		var/turf/cur = start
		var/dir
		if (start.x == end.x)
			var/d = end.y-start.y
			if(d) d = d/abs(d)
			end = get_turf(locate(end.x,end.y+d,end.z))
			dir = "v"
		else
			var/d = end.x-start.x
			if(d) d = d/abs(d)
			end = get_turf(locate(end.x+d,end.y,end.z))
			dir = "h"

		var/can_place = 1
		while (cur!=end && can_place)
			if(cur.density == 1)
				can_place = 0
			else if (isspaceturf(cur))
				can_place = 0
			else
				for(var/obj/O in cur)
					if(!istype(O, /obj/item/tape) && O.density)
						can_place = 0
						break
			cur = get_step_towards(cur,end)
		if (!can_place)
			to_chat(usr, span_notice("你不能把\the [src]塞进那里！"))
			return

		cur = start
		var/tapetest = 0
		while (cur!=end)
			for(var/obj/item/tape/Ptest in cur)
				if(Ptest.icon_state == "[Ptest.icon_base]_[dir]")
					tapetest = 1
			if(tapetest != 1)
				var/obj/item/tape/P = new tape_type(cur)
				P.icon_state = "[P.icon_base]_[dir]"
			cur = get_step_towards(cur,end)
	//is_blocked_turf(var/turf/T)
		to_chat(usr, span_notice("你放置好了[src]。")	)

/obj/item/tool/taperoll/afterattack(atom/A, mob/user as mob, proximity)
	if (proximity && istype(A, /obj/machinery/door/airlock))
		var/turf/T = get_turf(A)
		var/obj/item/tape/P = new tape_type(T.x,T.y,T.z)
		P.forceMove(locate(T.x,T.y,T.z))
		P.icon_state = "[src.icon_base]_door"
		P.layer = ABOVE_WINDOW_LAYER
		to_chat(user, span_notice("你放置好了[src]。"))

/obj/item/tape/proc/crumple()
	if(!crumpled)
		crumpled = 1
		icon_state = "[icon_state]_c"
		name = "皱巴巴的[name]"

/obj/item/tape/proc/on_cross(datum/source, atom/movable/AM, oldloc, oldlocs)
	SIGNAL_HANDLER
	if(!lifted && ismob(AM))
		var/mob/M = AM
		if(!allowed(M))	//only select few learn art of not crumpling the tape
			if(ishuman(M))
				to_chat(M, span_warning("你不应该越过[src]..."))
			crumple()

/obj/item/tape/attackby(obj/item/I, mob/user, params)
	. = ..()
	breaktape(I, user)

/obj/item/tape/attack_hand(mob/living/user)
	. = ..()
	if(.)
		return
	if (user.a_intent == INTENT_HELP && allowed(user))
		user.visible_message(span_notice("[user]抬起[src]，允许通行。"))
		crumple()
		lifted = TRUE
		addtimer(VARSET_CALLBACK(src, lifted, FALSE), 20 SECONDS)
	else
		breaktape(null, user)


/obj/item/tape/proc/breaktape(obj/item/W as obj, mob/user as mob)
	if(user.a_intent == INTENT_HELP && ((!can_puncture(W) && src.allowed(user))))
		to_chat(user, "你无法用那个破坏[src]！")
		return
	user.visible_message(span_notice("[user] 砸坏了 [src]！"))

	var/dir[2]
	var/icon_dir = src.icon_state
	if(icon_dir == "[src.icon_base]_h")
		dir[1] = EAST
		dir[2] = WEST
	if(icon_dir == "[src.icon_base]_v")
		dir[1] = NORTH
		dir[2] = SOUTH

	for(var/i=1;i<3;i++)
		var/N = 0
		var/turf/cur = get_step(src,dir[i])
		while(N != 1)
			N = 1
			for (var/obj/item/tape/P in cur)
				if(P.icon_state == icon_dir)
					N = 0
					qdel(P)
			cur = get_step(cur,dir[i])

	qdel(src)



