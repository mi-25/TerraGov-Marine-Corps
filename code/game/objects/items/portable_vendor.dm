...
/obj/item/portable_vendor
	name = "\improper 自动存储公文包"
	desc = "一个手提箱大小的自动化存储与检索系统。设计用于高效存储并有选择地分发小型物品。"
...
/obj/item/portable_vendor/proc/malfunction()
	var/turf/T = get_turf(src)
	T.visible_message(span_warning("[src] 的内部组件碎裂，剧烈颤抖！"))
	broken = 1
	STOP_PROCESSING(SSobj, src)
	update_appearance()
...
/obj/item/portable_vendor/corporate
	name = "\improper 纳米传讯自动存储公文包"
	desc = "一个手提箱大小的自动化存储与检索系统。设计用于高效存储并有选择地分发小型物品。这个箱子的侧面印有纳米传讯的徽标。"
