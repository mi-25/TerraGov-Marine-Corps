/obj/item/taperecorder
	desc = "一种能够录制长达一小时对话并回放的设备。回放时会自动翻译内容。"
	name = "通用录音机"
...
/obj/item/taperecorder/verb/record()
	...
	if(timerecorded < 3600 && playing == 0)
		...
		to_chat(usr, span_notice("要么是[src]的内存已满，要么它正在回放记忆。"))
...
/obj/item/taperecorder/verb/stop()
	...
	if(recording == 1)
		...
		T.visible_message("<font color=Maroon><B>[src]</B>: 播放已停止。</font>")
		icon_state = "taperecorderidle"
		return
...
/obj/item/taperecorder/verb/clear_memory()
	...
	if(recording == 1 || playing == 1)
		to_chat(usr, span_notice("你无法在游玩或录制时清除记忆！"))
		return
	else
		if(storedinfo)	storedinfo.Cut()
		if(timestamp)	timestamp.Cut()
		timerecorded = 0
		to_chat(usr, span_notice("记忆已清除。"))
		return
...
/obj/item/taperecorder/verb/playback_memory()
	...
	if(usr.stat)
		return
	if(recording == 1)
		to_chat(usr, span_notice("录制时无法回放！"))
		return
	if(playing == 1)
		to_chat(usr, span_notice("你已经进入游戏了！"))
		return
	playing = 1
	icon_state = "taperecorderplaying"
	to_chat(usr, span_notice("游戏开始。"))
	for(var/i=1,timerecorded<3600,sleep(10 * (playsleepseconds) ))
		...
		if(length(storedinfo) < i+1)
			...
			sleep(1 SECONDS)
			T = get_turf(src)
			T.visible_message("<font color=Maroon><B>[src]</B>: 记录结束。</font>")
		else
...
/obj/item/taperecorder/verb/print_transcript()
	...
	if(usr.stat)
		return
	if(!canprint)
		to_chat(usr, span_notice("记录仪打印不了那么快！"))
		return
	if(recording == 1 || playing == 1)
		to_chat(usr, span_notice("你无法在游戏或录制时打印记录！"))
		return
...
/obj/item/taperecorder/attack_self(mob/user)
	if(recording == 0 && playing == 0)
		...
		if(timerecorded < 3600 && playing == 0)
			...
			to_chat(usr, span_warning("要么[src]的内存已满，要么它正在回放记忆。"))
	else
		if(usr.stat)
			to_chat(usr, "在你失去行动能力时不行。")
			return
		if(recording == 1)
			recording = 0
			timestamp+= timerecorded
