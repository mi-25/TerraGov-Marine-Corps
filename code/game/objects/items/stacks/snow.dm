...
/obj/item/stack/snow/attackby(obj/item/I, mob/user, params)
	...
	to_chat(user, span_notice("你从[src]那里取走了雪。"))
	ET.update_icon()
	use(transf_amt)
	return TRUE
	...
	if(isopenturf(target))
		...
		if(T.get_dirt_type() == DIRT_TYPE_SNOW)
			...
			if(snowy_turf.slayer >= 3)
				to_chat(user, "这片地面已经积满了雪。")
				return
			if(amount < 5)
				to_chat(user, span_warning("你需要5堆雪来覆盖地面。"))
				return
...
/obj/item/stack/snow/attack_self(mob/user)
	...
	if(T.get_dirt_type() != DIRT_TYPE_SNOW)
		to_chat(user, span_warning("你无法在此处建造雪路障！"))
		return
	...
	for(var/obj/O in user.loc) //Objects, we don't care about mobs. Turfs are checked elsewhere
		if(O.density)
			if(O.atom_flags & ON_BORDER)
				...
				to_chat(user, span_warning("你需要一个开阔、无障碍的区域来搭建沙袋路障！"))
				return
