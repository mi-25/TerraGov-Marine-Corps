...
/obj/item/storage/box/tgmc_mre
	name = "\improper 地球政府殖民地海军陆战队即食口粮"
	desc = "<B>说明：</B>使用最大火力提取食物。吃掉。\n\n盒子上印着一个正在咆哮的班长。\n'你会吃掉你的营养糊，而且你会喜欢它，菜鸟。'"
	icon_state = "mre1"
...
/obj/item/reagent_containers/food/snacks/protein_pack
	name = "地球政府殖民地海军陆战队蛋白质棒"
	desc = "你见过的最假的蛋白棒，有多种口味可选"
...
/obj/item/reagent_containers/food/snacks/protein_pack/som
	name = "火星之子蛋白质棒"
	desc = "你见过最假的蛋白棒，有多种口味"
	icon = 'icons/obj/items/food/mre.dmi'
	faction = FACTION_SOM

/obj/item/reagent_containers/food/snacks/req_pizza
	name = "\improper TGMC 列兵 吉姆·披萨"
	desc = "你觉得那是个披萨。你绝对不该吃这东西，但你可以把它卖掉赚一笔！虽然它看起来确实像个披萨，但制作它的第一个、也是唯一的主要活性成分是一块圆形金属板。也许在自动存储检索系统里放一段时间后味道会变好点？唉，是时候把它卖给太空里某个倒霉的顾客了。"
...
/obj/item/reagent_containers/food/snacks/mre_pack/meal1
	name = "\improper TGMC 即食餐（香蕉面包）"
	desc = "一片香蕉面包，涂上了奶油派酱。滑溜溜的组合。"
	icon_state = "MREa"
	filling_color = "#ED1169"
...
/obj/item/reagent_containers/food/snacks/mre_pack/meal4
	name = "\improper TGMC 即食餐（披萨）"
	desc = "茄子、胡萝卜和甜玉米，铺在奶酪和番茄酱上。"
	icon_state = "MREd"
...
/obj/item/reagent_containers/food/snacks/mre_pack/meal6
	name = "\improper TGMC 预制餐（豆腐）"
	desc = "匈牙利儿童手工制作的烧烤粘豆腐汉堡，他们相信在一个有士兵杀人而非杀动物的星系里。"
	icon_state = "MREf"
	list_reagents = list(/datum/reagent/consumable/nutriment = 8)
...
/obj/item/storage/box/pizza
	name = "食品配送箱"
	desc = "一个太空时代的食物储存装置，能够保持食物格外新鲜。实际上，它只是个盒子。"
...
/obj/item/storage/box/nt_mre
	name = "\improper 纳米传讯品牌单兵即食口粮"
	desc = "一份来自纳米传讯工业的预包装、长效食品盒。\n盒子上印有纳米传讯的标识，周围环绕着一句标语：\n<b>纳米传讯。打造更美好的午餐</b>"
	icon_state = "mre2"
...
/obj/item/belt_harness
	name = "枪背带"
	desc = "一条带夹扣的皮革枪带。应该能防止你弄丢武器，但愿如此。"
...
/obj/item/belt_harness/attackby(obj/item/I, mob/user, params)
	...
	if(huser.belt != src)
		to_chat(user, span_notice("你需要穿戴[src]才能在上面安装配件！"))
		return
	if(reequip_component)
		if(reequip_component.parent == I)
			detach_item(I, user)
			return
		to_chat(user, span_notice("[src] 已经连接了\a [reequip_component.parent]！"))
		return
	attach_item(I, user)
...
/obj/item/belt_harness/proc/attach_item(obj/item/to_attach, mob/user)
	...
	to_chat(user, span_notice("[src] 咔哒一声，你将 \the [to_attach] 挂载了上去。"))
	update_icon()
...
/obj/item/belt_harness/proc/detach_item(source)
	...
	if(ishuman(loc))
		to_chat(loc, span_notice("[src] 发出咔哒声，\the [reequip_component.parent] 从其上脱钩[reequip_component.parent.p_s()]。"))
		playsound(src,'sound/machines/click.ogg', 15, FALSE, 1)
...
/obj/item/belt_harness/marine
	name = "\improper M45型腰带式携行具"
	desc = "肩部佩戴的带子，带有夹扣，可固定大多数物品。应该能防止你丢失武器，但愿如此。"
...
/obj/item/belt_harness/marine/equipped(mob/user, slot)
	...
	if(slot == SLOT_BELT)
		playsound(src,'sound/machines/click.ogg', 15, FALSE, 1)
		to_chat(user, span_danger("!!记住将你的武器连接到背带上，否则无法使用！！"))

/obj/item/compass
	name = "指南针"
	desc = "一个小型指南针，使用时可以显示你的坐标。"
	icon_state = "compass"
	w_class = WEIGHT_CLASS_TINY
...
/obj/item/compass/attack_self(mob/living/user)
	...
	to_chat(user, span_notice("观察[src]后，你可以得知自己的大致坐标。") + span_bold(" LONGITUDE [target_turf.x]. LATITUDE [target_turf.y]."))
...
/obj/item/compass/afterattack(atom/target, mob/user, has_proximity, click_parameters)
	...
	to_chat(user, span_notice("根据你当前的位置，目标坐标为：") + span_bold(" LONGITUDE [target_turf.x]. LATITUDE [target_turf.y]."))
