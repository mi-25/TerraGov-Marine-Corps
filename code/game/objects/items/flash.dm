...
/obj/item/flash/attack(mob/living/M, mob/user)
	...
	if(broken)
		to_chat(user, span_warning("\The [src] 已损坏。"))
		return
	...
	//	flick("flash2", src)
		if(!issilicon(M))
			...
			user.visible_message(span_disarm("[user]用闪光弹闪瞎了[M]！"))
		else

			user.visible_message(span_notice("[user]用闪光弹过载了[M]的传感器！"))
	else

		user.visible_message(span_notice("[user] 未能用闪光灯致盲 [M]！"))
	...
	switch(times_used)
		if(0 to 5)
			...
			if(iscarbon(loc))
				...
				if(M.flash_act())
					M.Paralyze(20 SECONDS)
					M.visible_message(span_disarm("[M] 被闪光弹致盲了！"))
