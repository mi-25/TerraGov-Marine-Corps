...
/obj/item/explosive/grenade/flare
	name = "\improper M40 FLDP 手雷"
	desc = "地球政府殖民地海军陆战队标准信号弹，采用标准DP罐体结构。可装入任何榴弹发射器，或用手投掷。"
...
/obj/item/explosive/grenade/flare/attack_self(mob/user)
	if(!fuel)
		to_chat(user, span_notice("燃料已耗尽。"))
		return
	if(active)
		return

	user.visible_message(span_notice("[user] 激活了信号弹。"), span_notice("You depress the ignition button, activating it!"))
	turn_on(user)
...
/obj/item/explosive/grenade/flare/civilian
	name = "信号弹"
	desc = "一根纳米传讯标准应急信号棒。侧面有使用说明，写着'拉绳，发光'。"
	icon_state = "flare"
	worn_icon_state = "flare"

/obj/item/explosive/grenade/flare/cas
	name = "\improper M50 CFDP 信号弹"
	desc = "一种采用标准DP弹壳结构的TGMC信号弹。可用于任何榴弹发射器装填，或手动投掷。激活后可为CAS飞行员提供目标指示。"
