...
/obj/item/tool/analyzer/attack_self(mob/user as mob)
	...
	if(ongoing_weather)
		...
		to_chat(user, span_notice("下一场[ongoing_weather]将在[(ongoing_weather.next_hit_time - world.time)/10]秒后到来。"))
		if(ongoing_weather.aesthetic)
			to_chat(user, span_warning("[src]的气压计功能显示，下一场风暴将轻轻掠过。"))
	else
