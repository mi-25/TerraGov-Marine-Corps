...
/obj/item/weapon/energy/axe/suicide_act(mob/user)
	user.visible_message(span_danger("[user]将[name]挥向[user.p_their()]的头部！看起来[user.p_theyre()]试图自杀。"))
	return (BRUTELOSS|FIRELOSS)

/obj/item/weapon/energy/axe/attack_self(mob/user)
	active = !active
	if(active)
		to_chat(user, span_notice("斧头现在已充能。"))
		force = force_activated
		icon_state = "axe1"
		w_class = WEIGHT_CLASS_HUGE
		heat = 3500
	else
		to_chat(user, span_notice("这把斧头现在可以隐藏了。"))
		force = initial(force)
		icon_state = "axe0"
		w_class = WEIGHT_CLASS_HUGE
		heat = 0
