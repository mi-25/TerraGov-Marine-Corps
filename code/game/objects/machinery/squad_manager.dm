...
/datum/squad_manager/ui_act(action, list/params)
	...
	if(!ismarineleaderjob(user.job)  && !issommarineleaderjob(user.job))
		to_chat(user, span_danger("必须是班长才能编辑小队。"))
		return

	if(user.assigned_squad?.type == /datum/squad) //means its a generated squad
		to_chat(user, span_danger("你已加入一个已创建的小队。"))
		return
	...
	user.balloon_alert(user, "'[new_name]'已创建")
