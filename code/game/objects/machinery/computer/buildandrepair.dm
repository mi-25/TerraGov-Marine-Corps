	...
	switch(state)
		if(0)
			if(iswrench(I))
				...
				to_chat(user, span_notice("你将框架拧紧到位。"))
				anchored = TRUE
				state = 1
		...
		if(1)
			if(iswrench(I))
				...
				to_chat(user, span_notice("你解开了框架。"))
				anchored = FALSE
				state = 0
				...
				to_chat(user, span_notice("你取下了电路板。"))
				state = 1
				icon_state = "0"
		...
		if(2)
			if(isscrewdriver(I) && circuit)
				playsound(loc, 'sound/items/screwdriver.ogg', 25, 1)
				to_chat(user, span_notice("你解开了电路板。"))
				state = 1
				icon_state = "1"
				...
				if(C.get_amount() < 5)
					to_chat(user, span_warning("你需要五卷电线才能将它们添加到框架上。"))
					return
				to_chat(user, span_notice("你开始往框架上添加电缆。"))
				...
				to_chat(user, span_notice("你将电缆连接到框架上。"))
				state = 3
				icon_state = "3"
		if(3)
			if(iswirecutter(I))
				playsound(loc, 'sound/items/wirecutter.ogg', 25, 1)
				to_chat(user, span_notice("你取下了电缆。"))
				state = 2
				icon_state = "2"
				...
				if(G.get_amount() < 2)
					to_chat(user, span_warning("你需要两块玻璃来安装玻璃面板。"))
					return
				playsound(loc, 'sound/items/deconstruct.ogg', 25, 1)
				...
				to_chat(user, span_notice("你装上了玻璃面板。"))
				state = 4
				icon_state = "4"
		if(4)
			if(iscrowbar(I))
				...
				playsound(loc, 'sound/items/screwdriver.ogg', 25, 1)
				to_chat(user, span_notice("你连接了监视器。"))
