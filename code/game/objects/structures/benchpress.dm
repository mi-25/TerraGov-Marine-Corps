/obj/structure/benchpress
	name = "力量训练凳"
	desc = "光是看着这东西就让你感到疲惫。<br>左键点击卧推，右键调整重量。"
...
/obj/structure/benchpress/proc/do_workout_set(mob/living/user)
	...
	user.visible_message("<B>[user] 正在[bragmessage]！</B>")
...
/obj/structure/benchpress/proc/undo_buff(datum/weakref/user_ref)
	...
	user.set_skills(user.skills.modifyRating(unarmed=-1))
	to_chat(user, span_boldnotice("你感觉自己的身体素质不如从前了！"))
