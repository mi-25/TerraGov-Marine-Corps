	...
	if(!(to_check_flags & ABILITY_USE_SOLIDOBJECT))
		...
		if(isclosedturf(current_turf))
			if(!silent)
				...
				to_chat(owner, span_warning("我们无法在固体物体内进行轨道轰炸！"))
			return FALSE
