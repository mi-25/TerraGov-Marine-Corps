...
/client/proc/update_exp_list(minutes, announce_changes = FALSE)
	...
	if(holder && !holder.deadmined)
		...
		if(announce_changes)
			to_chat(src,span_notice("你获得了：[minutes] 点管理经验！"))
	...
	if(isliving(mob))
		...
		if(mob.stat != DEAD)
			...
			if(announce_changes)
				to_chat(src,span_notice("你获得了：[minutes] 点生存经验值！"))
			if(living_mob.job)
				if(!istype(living_mob.job, /datum/job/fallen))
					...
					if(announce_changes)
						to_chat(src,span_notice("你获得了：[minutes] [living_mob.job] 经验值！"))
				else
			...
			if(announce_changes)
				to_chat(src,span_notice("你获得了：[minutes] 幽灵经验值！"))
