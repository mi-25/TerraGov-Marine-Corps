...
/datum/element/scalping/proc/on_attack(datum/source, mob/living/M, mob/living/user)
	...
	if(M.a_intent == INTENT_HARM)
		return NONE
	M.visible_message(span_notice("[user]开始用\the [source]撕扯[M]") ,span_notice("You start hacking away at [M] with \the [source]"))
	if(!do_after(user, 2 SECONDS, NONE, M))
		return NONE
	M.visible_message(span_danger("[user] 残忍地剥下了 [M] 的头皮！"), span_danger("You brutally scalp [M] with \the [source]!"))
